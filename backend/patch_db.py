import sqlite3
import os

db_path = os.path.join(os.path.dirname(__file__), "educore.db")
if os.path.exists(db_path):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(vehicles)")
    columns = [col[1] for col in cursor.fetchall()]
    
    if "is_tracking" not in columns:
        print("Adding is_tracking to vehicles table")
        cursor.execute("ALTER TABLE vehicles ADD COLUMN is_tracking BOOLEAN DEFAULT 0")
    if "current_latitude" not in columns:
        print("Adding current_latitude to vehicles table")
        cursor.execute("ALTER TABLE vehicles ADD COLUMN current_latitude NUMERIC(9,6)")
    if "current_longitude" not in columns:
        print("Adding current_longitude to vehicles table")
        cursor.execute("ALTER TABLE vehicles ADD COLUMN current_longitude NUMERIC(9,6)")
    if "last_location_update" not in columns:
        print("Adding last_location_update to vehicles table")
        cursor.execute("ALTER TABLE vehicles ADD COLUMN last_location_update DATETIME")
    
    # Patch students table
    cursor.execute("PRAGMA table_info(students)")
    student_columns = [col[1] for col in cursor.fetchall()]
    
    missing_student_cols = {
        "profile_photo_url": "VARCHAR(500)",
        "blood_group": "VARCHAR(10)",
        "nationality": "VARCHAR(50) DEFAULT 'Indian'",
        "religion": "VARCHAR(50)",
        "aadhaar_number": "VARCHAR(255)",
        "previous_school": "VARCHAR(255)",
        "tc_number": "VARCHAR(50)",
        "admission_date": "DATE",
        "address_line1": "VARCHAR(255)",
        "address_line2": "VARCHAR(255)",
        "city": "VARCHAR(100)",
        "state": "VARCHAR(100)",
        "pincode": "VARCHAR(15)",
        "alternate_phone": "VARCHAR(20)"
    }
    
    for col_name, col_type in missing_student_cols.items():
        if col_name not in student_columns:
            print(f"Adding {col_name} to students table")
            cursor.execute(f"ALTER TABLE students ADD COLUMN {col_name} {col_type}")
    
    # Patch fee_structures table
    cursor.execute("PRAGMA table_info(fee_structures)")
    fee_struct_cols = [col[1] for col in cursor.fetchall()]
    if "class_id" not in fee_struct_cols:
        cursor.execute("ALTER TABLE fee_structures ADD COLUMN class_id VARCHAR(36)")
    if "fee_head" not in fee_struct_cols:
        cursor.execute("ALTER TABLE fee_structures ADD COLUMN fee_head VARCHAR(50) DEFAULT 'Tuition Fee'")
    if "due_date" not in fee_struct_cols:
        cursor.execute("ALTER TABLE fee_structures ADD COLUMN due_date DATE")

    # Create fee_invoices table if missing
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS fee_invoices (
        id VARCHAR(36) PRIMARY KEY,
        invoice_number VARCHAR(50) UNIQUE NOT NULL,
        student_id VARCHAR(36) NOT NULL,
        fee_structure_id VARCHAR(36),
        title VARCHAR(150) NOT NULL,
        amount_due NUMERIC(10,2) NOT NULL,
        amount_paid NUMERIC(10,2) DEFAULT 0.0,
        due_date DATE NOT NULL,
        status VARCHAR(20) DEFAULT 'unpaid',
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME,
        is_deleted BOOLEAN DEFAULT 0,
        deleted_at DATETIME,
        deleted_by VARCHAR(36),
        FOREIGN KEY(student_id) REFERENCES students(id),
        FOREIGN KEY(fee_structure_id) REFERENCES fee_structures(id)
    )
    """)

    # Patch fee_payments table
    cursor.execute("PRAGMA table_info(fee_payments)")
    fee_pay_cols = [col[1] for col in cursor.fetchall()]
    if "invoice_id" not in fee_pay_cols:
        cursor.execute("ALTER TABLE fee_payments ADD COLUMN invoice_id VARCHAR(36)")
    if "payment_method" not in fee_pay_cols:
        cursor.execute("ALTER TABLE fee_payments ADD COLUMN payment_method VARCHAR(36) DEFAULT 'cash'")
    if "transaction_reference" not in fee_pay_cols:
        cursor.execute("ALTER TABLE fee_payments ADD COLUMN transaction_reference VARCHAR(100)")
    if "paid_by" not in fee_pay_cols:
        cursor.execute("ALTER TABLE fee_payments ADD COLUMN paid_by VARCHAR(100)")

    conn.commit()
    conn.close()
    print("Database patched successfully!")
else:
    print("No database found to patch.")
