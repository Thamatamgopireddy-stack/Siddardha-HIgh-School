...
import {
  ...
  Mail, BarChart3, Settings, Code2, ChevronLeft, Key, Sparkles,
} from 'lucide-react'
...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/portal-access', label: 'Portal Access', icon: Key },
...
function NavItem({ to, label, icon: Icon, collapsed, onClick }: { to: string; label: string; icon: React.ElementType; collapsed: boolean; onClick?: () => void }) {
  return (
    <NavLink
      to={to}
...
function PermissionNavItem({ permission, ...props }: { permission?: string; to: string; label: string; icon: React.ElementType; collapsed: boolean; onClick?: () => void }) {
    ...
    return (
      <aside className={cn('flex h-full flex-col bg-sidebar text-white transition-all', collapsed ? 'w-16' : 'w-64')}>
        ...
        <nav className="flex-1 space-y-1 p-3">
          <NavItem to="/dashboard" label="Dashboard" icon={LayoutDashboard} collapsed={collapsed} onClick={handleLinkClick} />
        </nav>
      </aside>
    )
  }
  ...
  return (
    <aside className={cn('flex h-full flex-col bg-sidebar text-white transition-all', collapsed ? 'w-16' : 'w-64')}>
      <div className="flex h-16 items-center justify-between border-b border-white/10 px-4">
        {!collapsed && (
          <div>
            <p className="font-semibold">Siddardha High School</p>
          </div>
        )}
        <button onClick={toggle} className="hidden lg:block rounded p-1 hover:bg-white/10"><ChevronLeft className={cn('h-5 w-5 transition', collapsed && 'rotate-180')} /></button>
      </div>
          ...
          <div key={group.label} className="mb-4">
            ...
            <div className="space-y-1">
              {group.items.map((item) => (
                <PermissionNavItem key={item.to} {...item} collapsed={collapsed} onClick={handleLinkClick} />
              ))}
            </div>
          </div>
        ))}
