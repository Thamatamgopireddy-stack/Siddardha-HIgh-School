...
import { Check, AlertCircle, Save, Info, Upload } from 'lucide-react'
...
export function AttendancePage() {
  ...
  const bulkImportAttendanceMutation = useBulkImportAttendanceExcel()
  ...
  useEffect(() => {
    ...
    students.forEach((s) => {
      // Find existing record safely comparing student_id
      const exist = existingRecords?.find((r: any) => String(r.student_id).toLowerCase() === String(s.id).toLowerCase())
      let savedStatus = 'present'
      if (exist && exist.status) {
        savedStatus = typeof exist.status === 'object' ? exist.status.value || String(exist.status) : String(exist.status)
      }
      initialStates[s.id] = savedStatus
    })
  ...
  return (
    <PageWrapper
      ...
      actions={
        ...
        >
          <Upload className="h-4 w-4" /> Import Attendance Excel
        </button>
      }
    >
      <ExcelImportModal
        ...
        title="Attendance Sheet Excel Import"
        description="Upload an Excel sheet (.xlsx, .xls) or CSV file with daily student attendance markers."
        templateUrl="/attendance/bulk-template"
        templateFileName="attendance_import_template.xlsx"
        onImport={async (file) => {
          if (!selectedSection || !selectedAcademicYear) {
            throw new Error('Please select Academic Year, Class, and Section before uploading attendance sheet.')
          }
          return await bulkImportAttendanceMutation.mutateAsync({
            file,
