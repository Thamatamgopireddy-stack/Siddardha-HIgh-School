...
function AdminDashboard() {
  ...
  const totalStudents = stats?.total_students !== undefined ? String(stats.total_students) : '...'
  const totalTeachers = stats?.total_teachers !== undefined ? String(stats.total_teachers) : '...'
  const feeCollected = stats?.fee_collected_month !== undefined ? `₹${stats.fee_collected_month}` : '...'
  const attendanceRate = stats?.attendance_rate !== undefined ? `${stats.attendance_rate}%` : '...'

  return (
    <>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Total Students" value={totalStudents} subtext="Active in database" icon={<Users className="h-5 w-5" />} />
        <StatCard label="Total Teachers" value={totalTeachers} subtext="Including staff" icon={<GraduationCap className="h-5 w-5" />} color="bg-purple-600" />
        <StatCard label="Fee Collected (Month)" value={feeCollected} trend={8.2} icon={<Wallet className="h-5 w-5" />} color="bg-success" />
        <StatCard label="Avg Attendance Today" value={attendanceRate} subtext={`${stats?.present_today || 0} present`} icon={<CalendarCheck className="h-5 w-5" />} color="bg-warning" />
      </div>
