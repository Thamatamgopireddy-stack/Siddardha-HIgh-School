...
export function useBulkImportStudents() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async (payload: { file: File; academicYearId: string; sectionId?: string; dryRun?: boolean }) => {
      const formData = new FormData()
      formData.append('file', payload.file)
      formData.append('academic_year_id', payload.academicYearId)
      formData.append('dry_run', payload.dryRun !== undefined ? String(payload.dryRun) : 'true')
      if (payload.sectionId) {
        formData.append('section_id', payload.sectionId)
      }
      const { data } = await api.post<APIResponse<any>>('/students/bulk-import', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      return data.data
    },
...
export function usePublishExam(examId: string) {
  ...
  return useMutation({
    mutationFn: async (is_published: boolean) => {
      const { data } = await api.put<APIResponse<any>>(`/exams/${examId}/publish`, { is_published })
      return data.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['exams'] })
...
export function useMyExamResults() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<APIResponse<any[]>>('/exams/my-results')
      return data.data || []
    },
  })
}
...
export function useCreateFeePayment() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['my-fee-dues'] })
      queryClient.invalidateQueries({ queryKey: ['student-fee-balances'] })
      queryClient.invalidateQueries({ queryKey: ['outstanding-report'] })
      queryClient.invalidateQueries({ queryKey: ['dashboard-stats'] })
    },
  })
}
...
export function useFeeInvoices(classId?: string, sectionId?: string, statusFilter?: string, studentId?: string) {
  return useQuery({
    queryKey: ['fee-invoices', classId, sectionId, statusFilter, studentId],
    queryFn: async () => {
      const { data } = await api.get<APIResponse<any[]>>('/fees/invoices', {
        params: {
...
export function useGenerateBulkInvoices() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async (payload: { fee_structure_id: string; class_id: string; section_id?: string; due_date?: string }) => {
      const { data } = await api.post<APIResponse<any>>('/fees/invoices/generate-bulk', payload)
      return data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['fee-invoices'] })
...
export function useMyFeeDues() {
  return useQuery({
    queryKey: ['my-fee-dues'],
    queryFn: async () => {
      const { data } = await api.get<APIResponse<any>>('/fees/my-dues')
      return data.data
    },
  })
}

export function useOutstandingReport(classId?: string, sectionId?: string, startDate?: string, endDate?: string) {
  return useQuery({
    queryKey: ['outstanding-report', classId, sectionId, startDate, endDate],
    queryFn: async () => {
      const { data } = await api.get<APIResponse<any>>('/fees/outstanding-report', {
        params: {
          class_id: classId || undefined,
          section_id: sectionId || undefined,
...
export function useDashboardStats() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<APIResponse<any>>('/fees/dashboard-stats')
      return data.data
    },
  })
}
